...
@PreventIframe("/testNodejsMessageExtension/config.html")
@PreventIframe("/testNodejsMessageExtension/action.html")
export default class TestNodejsMessageExtension implements IMessagingExtensionMiddlewareProcessor {
    ...
    public async onFetchTask(context: TurnContext, value: IMessagingExtensionActionRequest): Promise<MessagingExtensionResult | ITaskModuleResult> {
        ...
        return Promise.resolve<ITaskModuleResult>({
            ...
            value: {
                title: "Input form",
                url: `https://${process.env.HOSTNAME}/testNodejsMessageExtension/action.html`
            }
        });
